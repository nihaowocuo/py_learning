...
## Q79
装饰器其本质是否也是一个函数，只是作用是简化用户操作、集成功能性函数的调用？
类比：打游戏想开外挂时，装饰器会替我完成“打开外挂、关闭外挂”等前置/后置操作？


## Q80
当我的函数名和全局变量名或局部变量名一致时，是否会冲突？


## Q81
为何在 C 中整数只占 4 字节（int c = 12），而 Python 中 sys.getsizeof(42) 却占 28 字节？两者差别为何这么大？


## Q82
（接 Q81：int 在 py 中占 28 字节）问：类似于封装了一个类？内部含有多个属性与操作？


## Q83
（图）这一步中，是不是使用了闭包函数能够保存局部变量？把 `play_dnf` 的函数名传入为 `game`，此时又定义了内层函数 `inner`，当定义 `inner` 时调用了 `game()`，`game` 函数是否会被执行？还是说等外层函数返回 `inner` 后，由新的 `play_dnf` 接收，再由 `play_dnf()` 调用后 `game` 才会执行？


## Q84
（接 Q82：42 是 int 类的实例）问：x = 42，42 是一个 int 类的实例，那 x 又是什么东西？


## Q85
（接 Q84）问：x 存储的只是一个地址？而非像 C 语言中那样，对象就是类的实例对吗？


## Q86
（接 Q83 装饰器闭包）问：装饰器的本质就是把原有的功能性函数添加一些额外操作、然后重新封装为原函数名？这不就和文件修改一样吗？用修改后的文件删除源文件、再把修改后的文件名改为源文件名？


## Q87（2026-08-09）

装饰器和直接修改原函数有什么区别？装饰器不就是不断进行函数嵌套吗？给一个游戏制作外挂是不是类似的操作？

如果下载了一个已经被别人修改（嵌套了原函数）的外挂，我该在哪里再加自己的修改——是在他"已经嵌套了原函数的外层函数内"改，还是在他外挂函数之外再嵌套一层我自己的函数？这样嵌套会不会冗余、容易出问题？


## Q88（2026-08-09）

wrapper 是什么意思？在装饰器里 wrapper 这个词是指什么？


## Q89（2026-08-10）

@guanjia 即 play_dnf = guanjia(play_dnf) 时，传过去的 play_dnf 不只是函数名，还携带了它的参数——对吗？


## Q90（2026-08-10）

带参函数装饰器代码：play_dnf(username,password) 经 @guanjia 后，调用 play_dnf('admin','123456') 报错（inner 缺参 / 原 play_dnf 缺参）。用户认为此时把 play_dnf 的参数也传入管家 guanjia 了——对吗？


## Q91（2026-08-10）

请对以下带参装饰器代码逐行给出中文注释：guanjia(game) 内定义 inner(username,password) 并 game(username,password) 透传；@guanjia 装饰 play_dnf(username,password) 与 play_lol(username,password,hero)。


## Q92（2026-08-11）

检验我对 play_dnf = guanjia(play_dnf) 执行顺序的分析：右侧先执行 -> guanjia 调用 game=原函数 -> 定义 inner 未调用 -> return inner 使 play_dnf=inner -> 调用 play_dnf('admin','123456') 即调 inner。并解析 game(*args,**kwargs) 处为何要加参数、如何被加进去。
...
## Q95: 使用全局变量的目的就是能够不受到局部变量的影响、保存登录状态？
确认：装饰器里用全局变量 login_flag，是为了让登录状态在整个程序运行期内持久存在、且被四个 inner 共享。

## Q96: 后续完善需加入"退出登录"，把 login_flag 重新赋值为 False？
确认需求：增加一个 logout 操作，将全局 login_flag 重新置 False，使之后调用业务函数再次走登录流程。

## Q97: 迭代器省内存，是否只是内部存了类似指针的东西、记录位置、没有真实数据？
确认：迭代器是惰性/按需产出，不一次性物化整段数据；内部确实主要保存"来源 + 当前位置/状态"，而非全部元素。
